...
const computeGrid = (rects: Rect[]/* , vertical:  */) => {
  ...
  rows.push({
    top: rects[0].y - rects[0].height * .5,
    bottom: rects[0].y + rects[0].height * .5,
    idxs: [0],
  })
  columns.push({
    left: rects[0].x - rects[0].width * .5,
    right: rects[0].x + rects[0].width * .5,
    idxs: [0]
  })
  ...
  for (let i = 1; i < rects.length; i++) {
    ...
    for (let ri = 0; ri < rows.length; ri++) {
      const row = rows[ri]
      const bottomEdgeAboveRowsTop = rect.y + rect.height * .5 < row.top
      const topEdgeBelowRowsBottom = rect.y - rect.height * .5 > row.bottom
      const isOutsideRow = bottomEdgeAboveRowsTop || topEdgeBelowRowsBottom
      if (!isOutsideRow) {
        row.idxs.push(i)
        row.bottom = Math.max(rect.y + rect.height * .5, row.bottom)
        row.top = Math.min(rect.y - rect.height * .5, row.top)
        foundRow = true
        break
      }
    }
    ...
    for (let ci = 0; ci < columns.length; ci++) {
      ...
      const leftEdgeRightColumn = rect.x - rect.width * .5 > column.right
      const rightEdgeLeftColumn = rect.x + rect.width * .5 < column.left
      const isOutsideColumn = leftEdgeRightColumn || rightEdgeLeftColumn
      if (!isOutsideColumn) {
        column.idxs.push(i)
        column.left = Math.min(rect.x - rect.width * .5, column.left)
        column.right = Math.max(rect.x + rect.width * .5, column.right)
        foundColumn = true
        break
      }
    }
  ...
  .ctrls {
    ...
    .grid {
      ...
      > div {
        ...
        &.left:after { left: 0; }
        &.right:after { right: 0; }
        &.mid:after { left: 50%; transform: translateX(-50%); }
        &.center:after { top: 50%; transform: translateY(-50%); }
        &.mid.center:after { transform: translate(-50%, -50%); }
      }
...
const AlignUXPage: NextPage = () => {
  ...
  const selectOne = (stateNodes: typeof state['nodes'], idx: number, selected: boolean = true) => {
  ...
  const handleDrag = (e: React.DragEvent<HTMLDivElement>) => {
    state.nodes.forEach((node, idx) => {
      if (node.selected) {
        nodeRefs.current[idx].style.setProperty("left", `${e.pageX - dragStartX.current + node.x - node.width * .5}px`)
        nodeRefs.current[idx].style.setProperty("top", `${e.pageY - dragStartY.current + node.y - node.height * .5}px`)
      }
    })
  }
        ...
        state.nodes.map((node, idx) => (
          <div
            ...
            style={{
              ...
              left: `${node.x - node.width * .5}px`,
              top: `${node.y - node.height * .5}px`,
            }}
